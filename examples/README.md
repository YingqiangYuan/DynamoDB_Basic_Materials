...
- `dynamodb_basic_opeartions.one.api.one.bsm` configured with credentials that
  can manage DynamoDB in `us-east-1` (edit `profile_name` in
  `dynamodb_basic_opeartions/one/one_03_boto_ses.py` to point at your account).
...
- All DynamoDB calls run inside `with use_boto_session(Model, bsm):`
  using the `bsm` from `dynamodb_basic_opeartions.one.api.one`.
- All table names start with `dynamodb_basic_opeartions_` so
...
- [`cleanup_all_tables.py`](cleanup_all_tables.py) — list every
  `dynamodb_basic_opeartions_*` table and delete them after a confirmation prompt.
