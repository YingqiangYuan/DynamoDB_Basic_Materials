...
`dynamodb_basic_opeartions` repository. The user is here to *learn* what is in
...
2. Configure AWS credentials: edit `profile_name` in
   [dynamodb_basic_opeartions/one/one_03_boto_ses.py](dynamodb_basic_opeartions/one/one_03_boto_ses.py)
...
- [examples/cleanup_all_tables.py](examples/cleanup_all_tables.py) — utility: list and delete every `dynamodb_basic_opeartions_*` table after a `yes` prompt.
